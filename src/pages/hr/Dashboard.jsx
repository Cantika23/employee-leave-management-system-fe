 ...
 * dashboard karyawan. Sekarang juga menandai hari libur nasional
 * (dari `holidays`), jadi kalender ini merefleksikan cuti tim
 * DAN hari merah sekaligus.
 *
 * - Cuti karyawan (status approved): avatar inisial bertumpuk.
 * - Hari libur nasional: garis merah tipis di bawah tanggal.
 *
...
function CalendarModal({ onClose, holidays = [] }) {
  ...
  const selectedLeaves = selectedKey ? leavesByDay[selectedKey] || [] : []
  const selectedHoliday = selectedKey ? holidaysByDay[selectedKey] : null

  const monthSummary = useMemo(() => {
    const names = new Set()
    let dayCount = 0
    let holidayCount = 0
    Object.entries(leavesByDay).forEach(([key, items]) => {
    ...
    Object.keys(holidaysByDay).forEach((key) => {
      const [ky, km] = key.split('-').map(Number)
      if (ky === year && km - 1 === month) holidayCount += 1
    })
    return { people: names.size, days: dayCount, holidayCount }
  }, [leavesByDay, holidaysByDay, year, month])
    ...
    >
      <style>{`
        ...
        .acal-skel {
          ...
          animation: acal-shimmer 1.4s ease infinite;
          border-radius: 8px;
        }
        @keyframes acal-shimmer {
      ...
      >
        ...
        >
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            ...
            <div>
              <h2 style={{ margin: 0, fontSize: 16, lineHeight: 1.3 }}>Kalender</h2>
              <p className="hint" style={{ margin: '2px 0 0', fontSize: 12.5 }}>
                Cuti tim dan hari libur nasional
              </p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            >
              {Array.from({ length: 35 }).map((_, i) => (
                <div key={i} className="acal-skel" style={{ aspectRatio: '1 / 1' }} />
              ))}
            </div>
          ) : (
            <>
              ...
              >
                {cells.map((cell, idx) => {
                  ...
                  const classNames = [
                    'acal-daybtn',
                    isWeekend && !cell.muted ? 'acal-daybtn--weekend' : '',
                    isSelected ? 'acal-daybtn--selected' : '',
                  ]
                    .filter(Boolean)
                    .join(' ')
                  ...
                  return (
                    ...
                    >
                      <span
                        style={{
                          fontSize: 12.5,
                          fontWeight: hasLeave || holiday || isToday ? 700 : 500,
                          color: isToday
                            ? '#fff'
                            : hasLeave
                              ? '#2563eb'
                      ...
                      {holiday && (
                        <span
                          className="acal-holiday-bar"
                          style={isToday ? { background: '#fecaca' } : undefined}
                        />
                      )}
                    </button>
                  )
              ...
              {selectedDay && (selectedLeaves.length > 0 || selectedHoliday) && (
                <div
                  style={{
                    ...
                    padding: '10px 12px',
                    borderRadius: 10,
                    background: selectedHoliday
                      ? 'rgba(220, 38, 38, 0.06)'
                      : 'rgba(37, 99, 235, 0.06)',
                    border: `1px solid ${
                      selectedHoliday ? 'rgba(220, 38, 38, 0.18)' : 'rgba(37, 99, 235, 0.18)'
                    }`,
                  }}
                >
                  ...
                  {selectedHoliday && (
                    <div
                      style={{
                        ...
                        fontSize: 12.5,
                        color: '#b91c1c',
                        marginBottom: selectedLeaves.length > 0 ? 8 : 0,
                      }}
                    >
              ...
              >
                ...
                <span className="hint" style={{ fontSize: 12 }}>
                  {monthSummary.people > 0
                    ? `${monthSummary.people} orang cuti di ${monthSummary.days} hari`
                    : 'Tidak ada cuti bulan ini'}
                  {monthSummary.holidayCount > 0 &&
                    ` · ${monthSummary.holidayCount} hari libur nasional`}
                </span>
              </div>
            </>
          )}
